...
from word_replica.lab.visual import VisualOutcome, compare_visually
...
def _evaluate(source: Path, service, options, *, with_word: bool = False,
              with_visual: bool = False, qa_root: Path | None = None) -> Outcome:
    ...
    visual = visual_detail = None
    if with_visual:
        verdict = compare_visually(source, output, (qa_root or output.parent) / f"visual-{ref}")
        visual, visual_detail = verdict.outcome.value, verdict.detail
    ...
    return Outcome(
        ...
        visual=visual, visual_detail=visual_detail,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--visual", action="store_true",
                        help="also render both documents with Word and compare pages and pixels")
    args = parser.parse_args(argv)

    from word_replica.services.rebuild import RebuildService

    if (args.word or args.visual) and lock_is_held():
    ...
    for index, (source_name, source_ref) in enumerate(picks, start=1):
        ...
        if not path.exists():
            ...
            continue
        try:
            outcomes.append(_evaluate(
                path, service, options,
                with_word=args.word, with_visual=args.visual, qa_root=workspace,
            ))
        except Exception:
    ...
    rendered = [o for o in evaluated if o.visual]
    if rendered:
        counts = collections.Counter(o.visual for o in rendered)
        print("\nWHAT THE READER WOULD SEE")
        for name, count in counts.most_common():
            print(f"  {name:<22} {count:>4}")
        for kind in (VisualOutcome.PAGINATION_DIFFERS, VisualOutcome.PIXELS_DIFFER):
            examples = [o for o in rendered if o.visual == kind]
            if examples:
                print(f"\n  {kind}:")
                for outcome in examples[:8]:
                    print(f"    {outcome.source_ref:<44} {outcome.visual_detail}")
            ...
            json.dumps(
                {
                    ...
                    "visual_rendered": len([o for o in evaluated if o.visual]),
                    "visual_match": len(
                        [o for o in evaluated if o.visual == VisualOutcome.MATCH]
                    ),
                    "pagination_differs": len(
                        [o for o in evaluated if o.visual == VisualOutcome.PAGINATION_DIFFERS]
                    ),
